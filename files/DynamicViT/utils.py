...
def set_global_seed(seed: int, deterministic: bool = True) -> None:
    """
    Seed Python, NumPy, and PyTorch RNGs to keep experiments reproducible.
    """
    random.seed(seed)
    np.random.seed(seed)
...
def log_cli_command(
    ...
    *,
    ...
    """
    Append the current CLI invocation (with a timestamp) to `destination`.
    """
    if not destination:
        return
...
try:
    ...
    try:
        ...
        class SummaryWriter:  # minimal stub when tensorboard isn't installed
            def __init__(self, *args, **kwargs):
                print("TensorBoard is unavailable; logging will be disabled.")
...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None):
    ...
    if args.resume:
        if args.resume.startswith('https'):
            ...
            # Local checkpoints include optimizer / scaler state so we must allow full unpickling.
