...
def get_args_parser():
    ...
    parser.add_argument('--report-flops', action='store_true',
                        help='Report FLOPs (floating-point ops) for the configured model')
    parser.add_argument('--throughput', action='store_true',
                        help='Measure throughput with synthetic data and exit')
...
def main(args):
    ...
    KEEP_RATE1 = custom_rate if custom_rate is not None else [base_rate, base_rate ** 2, base_rate ** 3]
    KEEP_RATE2 = custom_rate if custom_rate is not None else [base_rate, base_rate - 0.2, base_rate - 0.4]
    profile_ratio = None
    ...
    if args.report_flops:
        model.eval()
        flops = calc_flops(model, args.input_size, ratios=profile_ratio)
