...
You can now tailor the keep ratio for each pruning layer independently via `--token-rate r1 r2 r3`. When unspecified, the script falls back to the original behaviour (`[base_rate, base_rate**2, base_rate**3]` for ViT/LVViT and `[base_rate, base_rate-0.2, base_rate-0.4]` for ConvNeXt/Swin).

To profile computational cost, append `--report-flops` to log GFLOPs every epoch. Use `--throughput` for the standalone synthetic throughput test (exits afterward), or `--epoch-throughput` to run the same synthetic test after each training epoch without exiting.
