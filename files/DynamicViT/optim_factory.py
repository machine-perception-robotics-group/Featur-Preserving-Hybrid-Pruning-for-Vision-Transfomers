...
def get_parameter_groups(model, weight_decay=1e-5, skip_list=(), get_num_layer=None, get_layer_scale=None, bone_lr_scale=0.01):
    ...
    for name, param in model.named_parameters():
        ...
        if group_name not in parameter_group_names:
            ...
            # backboneは、学習率を1/100にし、はじめの5エポックはパラメータ更新を行わない
            # backboneは、classifierやトークン予測モジュール以外の部分
            if group_name == 'decay':
                scale = bone_lr_scale
                fix_step = 5
...
def create_optimizer(args, model, get_num_layer=None, get_layer_scale=None, filter_bias_and_bn=True, skip_list=None, bone_lr_scale=0.01):
    ...
    if opt_lower == 'sgd' or opt_lower == 'nesterov':
        ...
        if NovoGrad is None:
            raise ImportError("timm.optim.novograd is unavailable in this environment")
        optimizer = NovoGrad(parameters, **opt_args)
    elif opt_lower == 'nvnovograd':
        if NvNovoGrad is None:
            raise ImportError("timm.optim.nvnovograd is unavailable in this environment")
